...
# Setup SimAlign
# Reference: 
# Authors: Jalili Sabet, Masoud  and Dufter, Philipp  and Yvon, Fran{\c{c}}ois  and Sch{\"u}tze, Hinrich
...
HSB_WORD_MEAN = np.loadtxt("/dss/dsshome1/0F/ge87fen2/PaSeMiLL/mean_vectors/mean_vector_hsb.txt")
DE_WORD_MEAN = np.loadtxt("/dss/dsshome1/0F/ge87fen2/PaSeMiLL/mean_vectors/mean_vector_de.txt")
...
def main(mapping_file_name: str, src_file_name: str, trg_file_name: str, model_path: str, output_file_name: str, window_size: int, min_segment_length: float, segment_detection_threshold: float, pair_filtering_threshold: float):
    ...
    if ".train." in mapping_file_name:
        gold_mapping_set = set(getMappedNumbers("/dss/dsshome1/0F/ge87fen2/PaSeMiLL/data/bucc_style_data/hsb-de/hsb-de.train.gold"))
    else:
        gold_mapping_set = set(getMappedNumbers("/dss/dsshome1/0F/ge87fen2/PaSeMiLL/data/bucc_style_data/hsb-de/hsb-de.test.gold"))
    ...
    myaligner = SentenceAligner(model=MODEL_TO_USE, token_type="bpe", matching_methods="a") #copied command from README SimAlign repo  --> changed model from mBERT to cis-lmu/glot500-base
    ...
    with open(stats_file_path, 'a', encoding='utf-8') as stats_file:
        ...
        for i, (src_num, trg_num) in enumerate(mapping_list):
            ...
            """
            alignments is a dict[str, list[tuple[int,int]]]
            e.g.:
            mwmf (Match): [(0, 0), (1, 1), (2, 2), (3, 3), (4, 4)]
            inter (ArgMax): [(0, 0), (1, 1), (2, 2), (3, 3), (4, 4)]
            itermax (IterMax): [(0, 0), (1, 1), (2, 2), (3, 3), (4, 4)]
            """
            # Argmax version is chosen for SimAlign because we need less mappings of tokens/ words (compared to Itermax)
            argmax_alignments_tuple_list_src = myaligner.get_word_aligns(src_sentence_words, trg_sentence_words)["inter"]     # copied following command from README SimAlign repo --> small changes
            argmax_alignments_tuple_list_trg = myaligner.get_word_aligns(trg_sentence_words, src_sentence_words)["inter"]     # copied following command from README SimAlign repo --> small changes

            sim_scores_src_to_trg = get_similarity_scores(argmax_alignments_tuple_list_src, src_word_lvl_vectors, trg_word_lvl_vectors,len(src_sentence_words))
            sim_scores_trg_to_src = get_similarity_scores(argmax_alignments_tuple_list_trg, trg_word_lvl_vectors, src_word_lvl_vectors,len(trg_sentence_words))
            ...
            # sim_scores_src_to_trg, src_sentence_words = delete_unaligned_stopwords(sim_scores_src_to_trg, src_sentence_words, argmax_alignments_tuple_list_trg, argmax_alignments_tuple_list_src, HSB_STOP_WORDS)
            sim_scores_trg_to_src, trg_sentence_words = delete_unaligned_stopwords(sim_scores_trg_to_src, trg_sentence_words, argmax_alignments_tuple_list_src, argmax_alignments_tuple_list_trg, GERMAN_STOP_WORDS)
            ...
            if min_segment_length > 1.0 and float(min_segment_length).is_integer():
                ...
                raise ValueError(
                    "Invalid min_segment_length. Please provide a float in (0,1] or an integer greater 1."
                )
            ...
            paired_segments = pair_segments(segment_list_src_indices, segment_list_trg_indices, argmax_alignments_tuple_list_src, 5)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run custom SimAlign post-processing on mined sentence pairs.")
