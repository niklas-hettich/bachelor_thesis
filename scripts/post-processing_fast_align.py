...
# TODO: cite the usage of fast_align
...
# assumption that every token/ word can be mapped just once by fast_align. This is not the case because of internal tokenization of the already tokenized input
...
def main(mapping_file_name: str, src_file_name: str, trg_file_name: str, model_path: str, output_file_name: str, window_size: int, min_segment_length: float, segment_detection_threshold: float, pair_filtering_threshold: float):
    ...
    # TODO before execution: file name/ path need to be changed perhaps
    all_sents_alignments_tuple_list_src = process_alignments_from_file("/dss/dsshome1/0F/ge87fen2/PaSeMiLL/fast_align_input_output/hsb-de/output_filtered_src_to_trg_0.1_v11.align") 
    all_sents_alignments_tuple_list_trg = process_alignments_from_file("/dss/dsshome1/0F/ge87fen2/PaSeMiLL/fast_align_input_output/hsb-de/output_filtered_trg_to_src_0.1_v11.align")
    ...
    for i, (src_num, trg_num) in enumerate(mapping_list):
        ...
        sim_scores_src_to_trg = get_similarity_scores(alignments_tuple_list_src, src_word_lvl_vectors, trg_word_lvl_vectors,len(src_sentence_words), i)
        alignments_tuple_list_trg = change_order_of_alignments_and_sort(alignments_tuple_list_trg, True)
        sim_scores_trg_to_src = get_similarity_scores(alignments_tuple_list_trg, trg_word_lvl_vectors, src_word_lvl_vectors,len(trg_sentence_words), i)
        ...
        # TODO: stopword function needs to be reimplemented to apply it also to Sorbian stopwords
...
# TODO: check parsing for mistakes
def parse_args():
